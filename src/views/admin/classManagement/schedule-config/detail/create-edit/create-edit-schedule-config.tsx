...
export default function CreateEditScheduleConfig() {
    const { handleSubmit, control, getValues, reset } = useForm<IFormScheduleConfig>({
        defaultValues: {
            classId: undefined,
            teacherId: undefined,
            weekDay: undefined,
    ...
    const { mutate: createScheduleConfig, isLoading: loadingCreate } = useCreateScheduleConfig();
    const { mutate: editScheduleConfig, isLoading: loadingEdit } = useEditScheduleConfig(id);
    const { data: detailScheduleConfig, isFetching: isLoadingDetail } = useGetDetailScheduleConfig(id, !!id)


    useEffect(() => {
        if (detailScheduleConfig) {
            const { classId, teacherId, weekDay, time, teacherRole } = detailScheduleConfig.data
                        ...
                        <Controller
                            ...
                            render={({ field: { onChange, value }, fieldState }) => (
                                <FormControl isRequired isInvalid={!!fieldState?.error}>
                                    ...
                                    <SelectComp options={optionWeekDay} value={value} onChange={onChange} placeholder="Chọn thứ" />
                                    <FormErrorMessage>
                                        {fieldState?.error?.message}
                                    </FormErrorMessage>
                                </FormControl>
                        ...
                        <Controller
                            ...
                            render={({ field: { ref, ...restField }, fieldState }) => (
                                <FormControl isRequired isInvalid={!!fieldState?.error}>
                                    ...
                                    <Input type={"time"} {...restField} placeholder="Chọn thời gian" />
                                    <FormErrorMessage>
                                        {fieldState?.error?.message}
                                    </FormErrorMessage>
                                </FormControl>
