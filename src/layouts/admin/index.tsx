			...
			if (route.children) {
				return route.children.flatMap((childRoute: any, index: number) => {
					if (childRoute?.children) {
						let listRouteDetail = childRoute.children.map((detail: any, index: number) => {
							return <Route path={route.layout + route.path + childRoute.path + detail.path} component={detail.component} key={index} />
						})
						const flagRoute = <Route path={route.layout + route.path + childRoute.path} component={childRoute.component} key={index} />
						listRouteDetail.push(flagRoute)
						return listRouteDetail
					}
					return <Route path={route.layout + route.path + childRoute.path} component={childRoute.component} key={index} />
				});
			}
