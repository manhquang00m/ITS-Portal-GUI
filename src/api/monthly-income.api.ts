...
export const deleteMonthlyIncome = (
  id: number
): Promise<IResponseDetailMonthlyIncome> => http.delete(`monthly-income/${id}`);

export const getStatusMonthlyIncome = (): Promise<IResponseOptions[]> =>
  http.get(`monthly-income/statuses`).then((response) => response?.data);
